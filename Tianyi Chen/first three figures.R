...
MDS_True_D =doMDS(True_D, doplot = T)
True_shuffled_D = sqrt(true_shuffled_dMV_square_London(tmax,tstar,p,q))

MDS_True_shuffled_D = doMDS(True_shuffled_D , doplot = T)
...
MDS_shuffle_0.2_hat <- doMDS(D2_shuffle_0.2_hat, doplot = T)

D2_shuffle_1 <- getD(df$shuffle_Xhat_1)
mds_hat_shuffled_dMV = doMDS(D2_shuffle_1, doplot= T)

HatdMV_D <- getD(df$xhat) 
mds_hat_dMV <- doMDS(HatdMV_D,doplot = T)
