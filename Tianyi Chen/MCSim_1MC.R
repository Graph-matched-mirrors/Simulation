...
n = 200
  ...
  for ( k in 1:3 ){
    ...
    tmp1[k+1]=find_slope_changepoint_with_plot(df.iso, doplot = T)$error
    tmp2[k+1]=find_slope_changepoint_with_plot(df.iso_shuffle, doplot = T)$error
    tmp3[k+1]=find_slope_changepoint_with_plot(df.iso_shuffle_GM_alltoone, doplot = T)$error
    tmp4[k+1]=find_slope_changepoint_with_plot(df.iso_shuffle_GM_pairwise, doplot = T)$error
  }
...
OFV=OFV_alltoone = OFV_pairwise = matrix(0,m,m)
